...
public static class UpdateChecker
{
        ...
        IProgress<int>?    progress = null)
    {
        ...
        // ── Refresh caches in parallel if expired ─────────────────────────
        var tasks = new List<Task>();

        // 항상 API 재조회 (캐시 TTL 무시) — 업데이트 버튼 클릭 시마다 최신 정보 보장
        ...
        List<InstalledMod> mods, ModioApi api, ModioCachedData cache)
    {
        ...
        // UUID → PublishHandle 역매핑 (batch 결과 적용용)
        // 중복 PublishHandle 허용 (번역 모드가 원본과 같은 ID를 가질 수 있음)
        var handleToUuid = new Dictionary<ulong, string>();
        foreach (var mod in mods)
            handleToUuid.TryAdd(mod.PublishHandle, mod.UUID);
        var handles = mods.Select(m => m.PublishHandle);

        var batch = await api.GetModsBatchAsync(handles);
    ...
    {
        Logger.Info($"UpdateChecker: refreshing Nexus cache for {mods.Count} mods");

        foreach (var mod in mods.Where(m => m.NexusModId.HasValue))
        {
            if (!api.CanMakeRequest()) break; // rate limit hit

            var data = await api.GetModInfoAsync(mod.NexusModId!.Value);
            if (data != null)
                cache.Mods[mod.UUID] = data;
        }

        cache.LastUpdated = DateTime.UtcNow;
        Logger.Info("UpdateChecker: Nexus cache refresh complete");
    }
